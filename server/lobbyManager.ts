...
export class LobbyManager {
  ...
  constructor(private readonly onRoundExpired?: (code: string) => void) {}
  ...
  startGame(code: string, socketId: string, preparedRoundsInput: unknown[]): MultiplayerLobbyState {
    const lobby = this.requireHostLobby(code, socketId);
    const preparedRounds = preparedRoundsInput.map(validatePreparedRound).slice(0, lobby.settings.rounds);
  ...
  private createCode(): string {
    let code = "";
    do {
      code = nanoid(4).replace(/[^a-z0-9]/gi, "").toUpperCase().padEnd(4, "X").slice(0, 4);
